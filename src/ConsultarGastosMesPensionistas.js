...
import './ConsultarGastosMesPensionistas.css';
...
const App = () => {
  ...
  const enviar = () => {
    ...
    axios
      .get(
        `https://apidatalake.tesouro.gov.br/ords/custos/tt/pensionistas?ano=${anoSelecionado}&mes=${mesSelecionado}`
      )
      .then((response) => {
        ...
        const columnMap = {
          ...
          in_sexo: 'Sexo',
          va_custo_pensionistas: 'Custo com pensionistas'
        };
        ...
        const filteredItems = response.data.items.map((item) => {
          ...
          Object.keys(item).forEach((key) => {
            if (!excludedColumns.includes(key)) {
              ...
              filteredItem[newKey] = key === 'va_custo_pensionistas' ? formatCurrency(item[key]) : item[key];
            }
          });
          return filteredItem;
        });
  ...
  return (
    <div className="gastos-mes-container">
      ...
      <nav className='nav-container'>
        ...
        <h2 className="titulo-1">Gastos com pensionistas por mês</h2>
